...
import RegulatorContract_artifacts from '../../build/contracts/Regulator.json';

// RegulatorContract is our usable abstraction, which we'll use through the code below.
var RegulatorContract = contract(RegulatorContract_artifacts);
...
window.App = {
  start: function() {
    ...
    // Bootstrap the RegulatorContract abstraction for Use.
    ...
    web3.eth.getAccounts(function(err, accs) {
      ...
      var temp; 
      RegulatorContract.deployed().then(function (instance) {
...
// var tollbooth = tollboothoperatorContract.at('0xd5c6b8d00d63a25588e18f740a357ac3201229cf');
// console.log("TOLLBOOTH" + tollbooth);
// console.log(tollbooth.isPaused());
// console.log(tollbooth.hashSecret("ABC"));
},
  createNewOperator: function() {
      var meta;
      RegulatorContract.deployed().then(function (instance) {
        meta = instance;
        var newOwner = document.getElementById("tollBoothOwner").value;
        var minDeposit = parseInt(document.getElementById("minDeposit").value);
        return meta.createNewOperator(newOwner, minDeposit, {from: account, gas: 4700000});
      }).then(function(result) {
        alert("Transaction successful!")
        var createOperatorEvent = meta.LogTollBoothOperatorCreated();
        createOperatorEvent.watch(function(error, result){
            if (!error)
                {
                  console.log("The contract address is " + result.args.newOperator);
                } else {
                    console.log(error);
                }
        });
  ...
  setVehicleType: function() {
    ...
    RegulatorContract.deployed().then(function(instance) {
      meta = instance;
      vehicleAddress = document.getElementById("vehicleAddress").value;
      vehType = parseInt(document.getElementById("vehicleType").value);
      return meta.setVehicleType(vehicleAddress,vehType, {from: account, gas: 470000});
    }).then(function(result) {
      alert("Transaction successful!")
      console.log(result);
    }).catch(function(e) {
