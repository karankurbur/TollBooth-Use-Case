...
window.App = {
  ...
  web3.eth.defaultAccount = account;
   ...
   console.log(tollBoothContract);
   tollBoothContract.isPaused(function(err, result) {
  if(!err) {
    console.log(result);
  }
});
   ...
   setMultiplier: function() {
    tollBoothContract = tollboothoperatorContract.at(tempAddress);
      ...
      var VehicleMult = parseInt(document.getElementById("VehicleMult").value);
      tollBoothContract.setMultiplier(VehicleType,VehicleMult, function(err, result){
      if(!err) {
        console.log(result);
        var receipt = web3.eth.getTransactionReceipt(result, function(err, result) {
          if(!err) {
            console.log(result);
          }
        });
  ...
  createTollBooth: function() {
    tollBoothContract = tollboothoperatorContract.at(tempAddress);
    var newBoothAddress = document.getElementById("newBoothAddress").value;
    tollBoothContract.addTollBooth(newBoothAddress, function(err, result) {
      if(!err) {
        console.log(result);
        var receipt = web3.eth.getTransactionReceipt(result, function(err, result) {
          if(!err) {
  ...
  setRoutePrice: function() {
      ...
      var exitAddress = document.getElementById("exitAddress").value;
      var routePrice = parseInt(document.getElementById("routePrice").value);
      tollBoothContract.setRoutePrice(entryAddress,exitAddress, routePrice, function(err, result) {
      if(!err) {
        console.log(result);
        var receipt = web3.eth.getTransactionReceipt(result, function(err, result) {
          if(!err) {
